...
export function HumanReviewPage({onBack}:{onBack:()=>void}) {
 ...
 <section className="reviewer-progress" aria-label="Progression dans les séances"><strong>Séance {sessionIndex+1} / {sessions.length}</strong><span>{session.id}</span></section>
 <section className={blocked?"reviewer-status is-blocked":"reviewer-status is-ready"}>{blocked?<ShieldAlert size={18}/>:<ShieldCheck size={18}/>}<div><strong>{blocked?"Revue bloquée":"Preuves disponibles"}</strong><span>{blocked?"Au moins un élément ne possède pas les deux preuves visuelles requises.":"Chaque élément de cette séance possède deux preuves visuelles déclarées."}</span></div></section>
 <div className="reviewer-items">{rows.map(({interaction,item})=>{const ready=hasEvidence(item);return <article className="reviewer-item" key={`${session.id}-${interaction.order}`}><div className="reviewer-item__meta"><strong>{interaction.order}. {interaction.mode}</strong><span>{interaction.itemId}</span></div>{item?<><bdi className="reviewer-arabic" dir="rtl" lang="ar">{item.arabicExact}</bdi><div className="reviewer-source"><span>{item.source.sourceId} · PDF {item.source.pdfPage} · imprimée {item.source.printedPage}</span><small>SHA-256 {item.integrity.utf8Sha256.slice(0,12)}…</small></div>{ready?<div className="reviewer-evidence"><a href={evidenceUrl(item.verification.evidence?.full)} target="_blank" rel="noreferrer">Page source <ExternalLink size={13}/></a><a href={evidenceUrl(item.verification.evidence?.crop)} target="_blank" rel="noreferrer">Zoom texte <ExternalLink size={13}/></a></div>:<div className="reviewer-blocked"><ShieldAlert size={14}/>Preuves visuelles manquantes — validation interdite</div>}</>:<div className="reviewer-blocked"><ShieldAlert size={14}/>Item absent du manifeste vérifié — validation interdite</div>}</article>})}</div>
