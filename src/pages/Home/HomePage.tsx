...
export function HomePage({ onStart, onOpenPath }: { onStart: (category: ExerciseCategory, targetSessionId?: string) => void; onOpenPath: () => void }) {
  ...
  const pendingCategory = unlocked.find(
  ...
  const nextCap = missionIsReview
    ...
    : hasIncompleteSession
      ...
      : waitingForDelayedCheck
        ...
        : pendingCategory
          ? `${CATEGORY_LABELS[pendingCategory]} est déverrouillée dans ton parcours, mais aucune séance contrôlée n’est encore active.`
          : nextCategory
