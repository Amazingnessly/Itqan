...
export function ReviewPage({ onStart }: { onStart: (category: ExerciseCategory) => void }) {
  ...
  return <main className="page review-page review-page--refined"><header className="review-header"><span className="section-kicker">Révision</span><h1>Révision ciblée</h1><p>Itqān reprend ce qui doit être stabilisé, pas une série choisie au hasard.</p></header><section className="review-priority-card"><div className="review-priority-card__topline"><span className="review-priority-icon"><Target size={18} strokeWidth={1.8} /></span><span>Priorité actuelle</span></div><h2>{plan.title}</h2><p>{plan.reason}</p><div className="review-metrics"><div><span>État</span><strong>{LEVEL_SYMBOLS[skill.level]} {LEVEL_LABELS[skill.level]}</strong></div><div><span>Précision</span><strong>{skill.totalAttempts ? `${accuracy} %` : "À établir"}</strong></div><div><span>Erreurs récentes</span><strong>{errors}</strong></div></div><button className="primary-cta" type="button" onClick={() => onStart(plan.category)}><RotateCcw size={17} />Lancer la reprise<ArrowRight size={17} /></button></section><section className="review-method-card"><div><ShieldCheck size={18} /><strong>Pourquoi cette reprise ?</strong></div><p>Une notion ne devient pas « maîtrisée » parce qu’une seule série a été réussie. Elle doit rester exacte dans plusieurs contextes et après un délai.</p></section><section className={`review-next${plan.dueNow ? " review-next--due" : ""}`}><CalendarClock size={17} /><div><strong>{plan.dueNow && skill.nextReviewAt ? "Contrôle de stabilité disponible" : "Révision différée"}</strong><p>{timingLabel ?? (skill.delayedCheckPassed ? "Le contrôle différé a été validé." : "Le prochain contrôle sera programmé après une première réussite.")}</p></div></section></main>;
}
