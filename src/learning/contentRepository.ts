...
export class ControlledContentRepository {
  ...
  constructor(batches: ControlledBatch[]) {
    for (const batch of batches) {
      for (const item of batch.items) {
        if (this.items.has(item.id)) throw new Error(`Duplicate controlled-content id: ${item.id}`);
  ...
  resolve(itemId: string, category: ExerciseCategory): ControlledContentItem {
    ...
    if (!item) throw new Error(`Unknown controlled-content id: ${itemId}`);
    ...
    if (!item.allowedExerciseTypes.includes(category)) {
      throw new Error(`Item ${itemId} is not authorized for ${category}`);
    }
    if (!item.eligibleForActiveLesson) throw new Error(`Item ${itemId} is not eligible for active lessons`);
    return item;
  }

  validateBlueprint(blueprint: ExerciseBlueprint): void {
    for (const session of blueprint.sessions) {
      if (session.interactionCount !== session.interactions.length) throw new Error(`Interaction count mismatch in ${session.id}`);
      for (const interaction of session.interactions) this.resolve(interaction.itemId, blueprint.category);
    }
  }
}
