...
const blueprintPath = "public/content/blueprints/linking-batch02.json";
const manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
const blueprint = JSON.parse(fs.readFileSync(blueprintPath, "utf8"));
const expectedSessions = ["LINKING-B02-S01", "LINKING-B02-S02", "LINKING-B02-S03"];

if (blueprint.category !== "linking") throw new Error("Unexpected blueprint category.");
if (JSON.stringify(blueprint.sessions.slice(0, 3).map((session) => session.id)) !== JSON.stringify(expectedSessions)) {
  throw new Error("Linking S01-S03 session prefix changed; refusing automatic activation.");
}
if (blueprint.unlockPolicy?.timingOnlyAfterPrecisionStability !== true) {
  throw new Error("Linking blueprint must require precision stability before timing.");
}

const targetIds = [...new Set(
  blueprint.sessions.slice(0, 3).flatMap((session) => session.interactions.map((interaction) => interaction.itemId)),
)];
if (targetIds.length !== 16) throw new Error(`Unexpected linking S01-S03 item set size: ${targetIds.length}.`);
...
for (const itemId of targetIds) {
  ...
  if (!item.allowedExerciseTypes?.includes("linking")) throw new Error(`Item is not authorized for linking: ${itemId}.`);
...
if (changed !== 0 && changed !== 1) throw new Error(`Unexpected linking activation delta: ${changed}; expected 1 or 0.`);
fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2) + "\n", "utf8");
console.log(`OK: linking S01-S03 use ${targetIds.length} safe controlled items; ${changed} newly activated.`);
