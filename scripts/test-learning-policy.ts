...
await run("Cloudflare adapter rejects invalid audio before network", async () => { let fetchCalls = 0; const provider = new CloudflareVoiceAssessmentProvider({ fetchImpl: (async () => { fetchCalls += 1; return new Response("unexpected"); }) as typeof fetch }); await assert.rejects(() => provider.assess({ ...voiceRequest, audio: new Blob([], { type: "audio/webm" }) }), /audio is empty/); await assert.rejects(() => provider.assess({ ...voiceRequest, audio: new Blob(["audio"], { type: "text/plain" }) }), /audio type is unsupported/); await assert.rejects(() => provider.assess({ ...voiceRequest, audio: new Blob([new Uint8Array(8 * 1024 * 1024 + 1)], { type: "audio/webm" }) }), /audio is too large/); assert.equal(fetchCalls, 0); });
await run("Cloudflare adapter requires a JSON response content type", async () => { const payload = JSON.stringify({ provider: "test", recognized: false }); const missing = new CloudflareVoiceAssessmentProvider({ fetchImpl: (async () => new Response(payload, { status: 200 })) as typeof fetch }); await assert.rejects(() => missing.assess(voiceRequest), /non-JSON response/); const text = new CloudflareVoiceAssessmentProvider({ fetchImpl: (async () => new Response(payload, { status: 200, headers: { "content-type": "text/plain" } })) as typeof fetch }); await assert.rejects(() => text.assess(voiceRequest), /non-JSON response/); const vendorJson = new CloudflareVoiceAssessmentProvider({ fetchImpl: (async () => new Response(payload, { status: 200, headers: { "content-type": "application/vnd.itqan.voice+json; charset=utf-8" } })) as typeof fetch }); assert.equal((await vendorJson.assess(voiceRequest)).provider, "test"); });
...
await run("Cloudflare adapter aborts a stalled request and safety falls back to manual control", async () => { let sawAbort = false; const fetchImpl = ((_: RequestInfo | URL, init?: RequestInit) => new Promise<Response>((_, reject) => { const signal = init?.signal; signal?.addEventListener("abort", () => { sawAbort = true; reject(new DOMException("Aborted", "AbortError")); }, { once: true }); })) as typeof fetch; const provider = new CloudflareVoiceAssessmentProvider({ fetchImpl, timeoutMs: 5 }); const guidance = await assessVoiceSafely(provider, voiceRequest); assert.equal(sawAbort, true); assert.equal(guidance.status, "unavailable"); });
await run("Cloudflare adapter rejects invalid timeout configuration", () => { assert.throws(() => new CloudflareVoiceAssessmentProvider({ timeoutMs: 0 }), /timeout must be positive/); });
