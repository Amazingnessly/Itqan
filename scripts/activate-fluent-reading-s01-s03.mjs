...
const blueprintPath = "public/content/blueprints/fluent_reading-batch02.json";
const manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
const blueprint = JSON.parse(fs.readFileSync(blueprintPath, "utf8"));
const expectedSessions = ["FLUENT_READING-B02-S01", "FLUENT_READING-B02-S02", "FLUENT_READING-B02-S03"];

if (blueprint.category !== "fluent_reading") throw new Error("Unexpected blueprint category.");
if (JSON.stringify(blueprint.sessions.slice(0, 3).map((session) => session.id)) !== JSON.stringify(expectedSessions)) {
  throw new Error("Fluent reading S01-S03 session prefix changed; refusing automatic activation.");
}
if (blueprint.unlockPolicy?.timingOnlyAfterPrecisionStability !== true) {
  throw new Error("Fluent reading blueprint must require precision stability before timing.");
}

const targetIds = [...new Set(
  blueprint.sessions.slice(0, 3).flatMap((session) => session.interactions.map((interaction) => interaction.itemId)),
)];
if (targetIds.length !== 10) throw new Error(`Unexpected fluent reading S01-S03 item set size: ${targetIds.length}.`);
...
for (const itemId of targetIds) {
  ...
  if (!item.allowedExerciseTypes?.includes("fluent_reading")) throw new Error(`Item is not authorized for fluent_reading: ${itemId}.`);
...
if (changed !== 0 && changed !== 7) throw new Error(`Unexpected fluent reading activation delta: ${changed}; expected 7 or 0.`);
fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2) + "\n", "utf8");
console.log(`OK: fluent reading S01-S03 use ${targetIds.length} safe controlled items; ${changed} newly activated.`);
